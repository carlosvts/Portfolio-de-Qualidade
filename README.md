...
|---|--------|------|------------|--------|
| #1 | Cadastro permite autenticação sem verificação de e-mail | Auth JWT | Crítica | Aberto |
| #2 | Usuário não administrador acessa endpoint restrito | Auth JWT | Crítica | Aberto |
| #3 | Sistema permite múltiplos check-ins por dia | Check-in / Pontos | Crítica | Aberto |
| #4 | Check-ins criados com status APPROVED em vez de PENDENTE | Check-in / Pontos | Crítica | Aberto |
| #5 | Endpoint de moderação de status inexistente | Check-in | Crítica | Aberto |
| #6 | IDOR: usuário acessa check-ins de outros usuários | Check-in | Crítica | Aberto |
| #7 | Código de configuração de upload em disco não utilizado | Check-in | Baixa | Aberto |
| #8 | Remoção de check-in não atualiza pontos nem contagem | Pontos | Crítica | Aberto |
| #9 | Sistema não respeita intervalo mínimo de 24h entre check-ins | Check-in | Crítica | Aberto |
...
#### [#1] Cadastro permite autenticação sem verificação de e-mail (RF-01 / RF-02)

**Localização:** API / Backend — POST /api/auth/register

**Passos para reproduzir:**
1. POST /api/auth/register com nome, e-mail e senha válidos
2. Observar a resposta

**Resultado esperado:** Usuário criado com status PENDENTE; nenhum token retornado antes da verificação.

**Resultado atual:** API retorna accessToken e refreshToken imediatamente; acesso a rotas protegidas liberado sem verificação.

**RCA:** Fluxo de verificação de e-mail não implementado no endpoint de cadastro.

**Severidade:** Crítica

---

#### [#2] Usuário não administrador consegue acessar endpoint restrito de gestão (RF-03)

**Localização:** API / Backend — GET /api/seasons

**Passos para reproduzir:**
1. Login com usuário MEMBER ou TRAINEE
...
**RCA:** Endpoint não aplica RBAC — controle baseado apenas em autenticação, sem verificação de role.

**Severidade:** Crítica

---

#### [#3] Sistema permite múltiplos check-ins por dia, acumulando pontos indevidamente (RF-04)

**Localização:** API / Backend — POST /api/checkins

**Passos para reproduzir:**
1. POST /api/checkins com imagem válida
2. Repetir a mesma requisição múltiplas vezes no mesmo dia

**Resultado esperado:** 400 ou 409 a partir do segundo check-in; pontos não incrementados.

**Resultado atual:** Todos aceitos com 201 Created; pontos acumulados a cada envio.

**RCA:** Ausência de validação de unicidade por usuário/data no endpoint de criação.

**Severidade:** Crítica

---

#### [#4] Check-ins são criados com status APPROVED em vez de PENDENTE (RF-05)
...
**Resultado esperado:** Check-in criado com status PENDENTE; pontos atribuídos somente após aprovação.

**Resultado atual:** Check-in criado com status APPROVED e pontos atribuídos imediatamente.

**RCA:** Backend define status como APPROVED na criação, ignorando o fluxo de moderação do RF-05.

**Severidade:** Crítica

---

#### [#5] Sistema não possui endpoint para alteração de status de check-ins (RF-05)

**Localização:** API / Backend — PATCH /checkins/:id

**Passos para reproduzir:**
1. Identificar check-in existente
2. PATCH /checkins/:id com body de atualização de status
...
**RCA:** Rota de moderação não implementada; fluxo de aprovação/rejeição ausente no backend.

**Severidade:** Crítica

---

#### [#6] Usuário consegue acessar check-ins de outros usuários — IDOR (RF-06)

**Localização:** API / Backend — GET /api/checkins/{id}

**Passos para reproduzir:**
1. Autenticar como TRAINEE ou MEMBER
2. Obter ID de check-in pertencente a outro usuário
3. GET /api/checkins/{id} com o token do usuário autenticado

**Resultado esperado:** 403 Forbidden ou 404 Not Found.

**Resultado atual:** 200 OK com dados completos do check-in alheio.

**RCA:** Backend não valida se o userId do check-in corresponde ao usuário autenticado — vulnerabilidade IDOR (Insecure Direct Object Reference).

**Severidade:** Crítica

---

#### [#7] Código de configuração de upload em disco não utilizado

**Localização:** Código-fonte — upload.js

**Descrição:** Existe configuração multer.diskStorage em upload.js que não é usada por nenhuma rota ativa. Uploads vão diretamente ao Cloudinary; nenhum arquivo foi gerado em app/uploads/ durante os testes.

**Impacto:** Código legado que pode causar confusão em manutenções ou comportamento inesperado se ativado incorretamente.

**Severidade:** Baixa

---

#### [#8] Remoção de check-in não atualiza saldo de pontos nem contagem de check-ins (RF-08)
...
3. Consultar novamente os dados do usuário

**Resultado esperado:** Pontos subtraídos; contagem decrementada; ranking atualizado.

**Resultado atual:** Check-in removido, porém pontos e contagem permanecem inalterados.

**RCA:** Lógica de deleção dissociada da consistência de dados — ausência de atualização transacional do saldo ao remover o registro.

**Severidade:** Crítica

---

#### [#9] Sistema não respeita intervalo mínimo de 24h entre check-ins (RF-09)

**Localização:** API / Backend — POST /api/checkins

**Passos para reproduzir:**
1. POST /api/checkins com imagem válida
2. Imediatamente repetir a requisição com o mesmo usuário

**Resultado esperado:** 400 ou 409 ao tentar novo check-in dentro de 24 horas.

**Resultado atual:** 201 Created; pontos atribuídos a cada envio.

**RCA:** Backend não implementa validação de intervalo baseada em createdAt.
